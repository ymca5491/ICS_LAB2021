...
uint32_t cache_read(uintptr_t addr) {
  ...
  cache[cache_num].mark = MARK(addr);
...
void cache_write(uintptr_t addr, uint32_t data, uint32_t wmask) {
  ...
  for (uint64_t i = asso_begin; i < asso_begin + asso_size; i++) {
    if (cache[i].dirty != -1 && cache[i].mark == mark) {
      *(uint32_t *)(cache[i].block + BLOCK_OFFSET(addr)) = data;
      cache[i].dirty = 1;
      return;
    }
  }
  ...
  mem_read(BLOCK_NUM(addr), cache[cache_num].block);
  *(uint32_t *)(cache[cache_num].block + BLOCK_OFFSET(addr)) = data;
}
